...
module _ {ℓOb ℓHom ℓTy ℓTm : Level} (C : Category ℓOb ℓHom) (cwf : Algebraic.CwF C ℓTy ℓTm) where
  ...
  record Σ-Structure-CwF  : Type ((ℓ-suc (ℓ-max ℓOb (ℓ-max ℓHom (ℓ-max ℓTy  ℓTm))))) where
    ...
    field
      ...
      ΣTmIsoInvNat : (A : Ty Γ)
                    ...
                    → PathP (λ i → Tm Δ (ΣTyNat A B σ i))
                            ...
                            (ΣTmIso (A [ σ ]Ty) (B [ σ ⁺ ]Ty) .Iso.inv (a [ σ ]Tm , subst (Tm Δ) (coerce A B a σ) (b [ σ ]Tm)))
